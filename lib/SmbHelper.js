      ...
      this.client.readdir(path, (err, files) => {
        if (err) {
          ...
          selectList = selectList.filter((item) => {
            const match = item.match(/(\d{8}T\d{6}Z)/);
          ...
          const deleteItem = (itemPath) => {
            return new Promise((res) => {
              this.client.unlink(itemPath, (err) => {
                if (err && (err.code === 'STATUS_FILE_IS_A_DIRECTORY' || err.code === 'STATUS_INVALID_PARAMETER')) {
                  this.client.readdir(itemPath, (readErr, files) => {
                    if (readErr || !files || files.length === 0) {
                      this.client.rmdir(itemPath, () => res());
                      return;
                    }
                    const pending = files.length;
                    for (const f of files) {
                      ((file, pendingRef) => {
                        this.client.unlink(`${itemPath}\\${file}`, () => {
                          pendingRef.count -= 1;
                          if (pendingRef.count === 0) this.client.rmdir(itemPath, () => res());
                        });
                      })(f, { count: pending });
                    }
                  });
          ...
          (async () => {
            for (let idx = 0; idx < selectList.length; idx += 1) {
              ...
              this.app.log(`removing SMB item ${item}`);
              await deleteItem(`${path}${item}`);
            }
            resolve(selectList);
          })();
        }
